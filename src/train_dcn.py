...
class CrossLayer(nn.Module):
    def __init__(self, feature_dim):
        super().__init__()
        self.weights = nn.Parameter(torch.randn(feature_dim, 1)*0.01)
        self.bias = nn.Parameter(torch.zeros(feature_dim))
        self.layer_norm = nn.LayerNorm(feature_dim)

    def forward(self, x0, xl):
        # xl+1 = x0 * (wTxl) + bl + xl, x0: (batch, feature_dim), xl: (batch, feature_dim)
        cross_term = xl @ self.weights # (batch, feature_dim @ feature_dim, 1) -> (batch, 1)
        cross_term = cross_term
        return self.layer_norm(x0 * cross_term + self.bias + xl)
...
class MultiHeadedCrossLayer(nn.Module):
    def __init__(self, feature_dim):
        super().__init__()
        self.weights1 = nn.Parameter(torch.randn(feature_dim, 1)*0.01)
        self.bias1 = nn.Parameter(torch.zeros(feature_dim))
        self.weights2 = nn.Parameter(torch.randn(feature_dim, 1)*0.01)
        self.bias2 = nn.Parameter(torch.zeros(feature_dim))
        self.weights3 = nn.Parameter(torch.randn(feature_dim, 1)*0.01)
        self.bias3 = nn.Parameter(torch.zeros(feature_dim))
    ...
    def forward(self, x0, xl):
        # xl+1 = x0 * (wTxl) + bl + xl, x0: (batch, feature_dim), xl: (batch, feature_dim)
        # cross_term = xl @ self.weights1 # (batch, feature_dim @ feature_dim, 1) -> (batch, 1)
        # cross_term = cross_term
        xl1 = x0 * (xl @ self.weights1) + self.bias1 
        xl2 = x0 * (xl @ self.weights2) + self.bias2 
        xl3 = x0 * (xl @ self.weights3) + self.bias3 
        xL = torch.concat([xl1, xl2, xl3], dim=1)
        out = self.linear(xL) + xl
        return self.layer_norm(out)
    
class akiClassifier(nn.Module):
    def __init__(self, feature_dim):
        super().__init__()
        self.cross_layer = CrossLayer(feature_dim)
        self.cross_layer1 = CrossLayer(feature_dim)
        self.cross_layer2 = CrossLayer(feature_dim)
        # self.cross_layer = MultiHeadedCrossLayer(feature_dim)
        # self.cross_layer1 = MultiHeadedCrossLayer(feature_dim)
        # self.cross_layer2 = MultiHeadedCrossLayer(feature_dim)
        self.deep_layer = DeepLayer(feature_dim)
        self.deep_layer1 = DeepLayer(feature_dim)
        self.deep_layer2 = DeepLayer(feature_dim)
    ...
    def forward(self, x):
        cross_x = self.cross_layer(x, x)
        cross_x1 = self.cross_layer1(x, cross_x)
        cross_x2 = self.cross_layer2(x, cross_x1)
        deep_x = self.deep_layer(x)
        deep_x1 = self.deep_layer(deep_x)
        deep_x2 = self.deep_layer(deep_x1)
        concat_x = torch.concat([cross_x2, deep_x2], dim=1)
        return self.classifier(concat_x)
...
def main():
    ...
    ap.add_argument("--out-prefix", default="artifacts/dcn_model/dcn")
    args = ap.parse_args()
    ...
    # ------------------------
    # Integrated Gradients feature importance (Captum, DCN version)
    # ------------------------
    try:
        from captum.attr import IntegratedGradients

        print("Computing feature importance with Integrated Gradients (DCN)...")
